...
from .common import EnvNewsChunkConfig
from .report_chunks import (
    create_generate_outline_chunk,
    create_prepare_request_chunk,
    create_render_report_chunk,
)
from .summary_chunks import create_summarize_column_chunk


def build_column_sub_flow(*, chunk_config: EnvNewsChunkConfig) -> TriggerFlow:
    flow = TriggerFlow(name="env-news-column-sub-flow")
    ...
    summarize_column_news = flow.chunk("summarize_column_news")(create_summarize_column_chunk(chunk_config))
    write_column = flow.chunk("write_column")(create_write_column_chunk(chunk_config))

    (
        flow.to(search_column_news)
        .to(pick_column_news)
        .to(summarize_column_news)
        .to(write_column)
        .end()
    )
    return flow
