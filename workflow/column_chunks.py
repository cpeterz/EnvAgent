...
def create_search_column_news_chunk(
    ...
    async def search_column_news(data: TriggerFlowRuntimeData) -> dict[str, Any] | None:
        ...
        try:
            search_results = await _search_news(
                config, logger, require_search_tool(data),
                column_outline, topic=str(request.get("topic") or ""),
            )
            for item in search_results:
                url = str(item.get("url") or "").strip()
                if url and url not in seen_urls:
                    seen_urls.add(url)
                    item["id"] = len(searched_news)
                    searched_news.append(item)
        except Exception as exc:
            logger.exception("[Column Search Failed] %s: %s", title, exc)

        if config.settings.rss.enabled:
            try:
                rss_tool = require_rss_tool(data)
                rss_results = await rss_tool.fetch_feeds(category=None)
                keywords = str(column_outline.get("search_keywords") or title).lower()
                for item in rss_results:
                    url = str(item.get("url") or "").strip()
                    item_title = str(item.get("title") or "").lower()
                    item_brief = str(item.get("brief") or "").lower()
                    if url and url not in seen_urls and _matches_keywords(keywords, item_title, item_brief):
                        seen_urls.add(url)
                        item["id"] = len(searched_news)
                        searched_news.append(item)
            except Exception as exc:
                logger.warning("[RSS Fetch Failed] %s: %s", title, exc)

        logger.info("[Total News Count] %s => %s", title, len(searched_news))
...
def create_pick_column_news_chunk(
    ...
    async def pick_column_news(data: TriggerFlowRuntimeData) -> dict[str, Any] | None:
        ...
        logger = require_logger(data)
        try:
            picked_news = await pick_news(config, column_outline, searched_news)
        except Exception as exc:
...
def create_write_column_chunk(
    ...
    async def write_column(data: TriggerFlowRuntimeData) -> dict[str, Any] | None:
        ...
        logger = require_logger(data)
        try:
            column_result = await _write_column(config, column_outline, context["summarized_news"])
        except Exception as exc:
...
def _matches_keywords(keywords: str, title: str, brief: str) -> bool:
    keyword_list = re.split(r"[\s,;，；]+", keywords)
    text = title + " " + brief
    return any(kw in text for kw in keyword_list if kw)
...
async def _search_news(
    ...
    *,
    ...
    if not query:
        return []
    queries = _build_search_queries(search_keywords=query, topic=topic)
    ...
    for candidate in queries:
        ...
        for raw in raw_results or []:
            ...
            if len(normalized_results) >= config.settings.search.max_results:
                break
        logger.info("[Search Attempt] %s => %d results", candidate, len(normalized_results))
        if len(normalized_results) >= config.settings.search.max_results:
            break
    return normalized_results


def _build_search_queries(*, search_keywords: str, topic: str) -> list[str]:
    queries: list[str] = []
    seen: set[str] = set()
...
def _extract_search_tokens(text: str) -> list[str]:
    tokens = re.findall(r"[一-鿿]+|[A-Za-z0-9][A-Za-z0-9._+-]*", text)
...
def _dedupe_tokens(tokens: list[str]) -> list[str]:
    ...
    for token in tokens:
        ...
        if normalized and lower_token not in seen:
...
async def _write_column(
    ...
    return {
        "title": column_outline.get("column_title", ""),
        "prologue": prologue,
...
def _build_fallback_prologue(
    ...
    col_title = column_outline.get("column_title", "")
    if is_chinese_language(config.settings.workflow.output_language):
        lead_titles = "，".join("《" + n["title"] + "》" for n in news_list[:3])
        return "本栏目围绕"" + col_title + ""整理了以下环境新闻重点内容：" + lead_titles + "。"
    lead_titles = ", ".join(n["title"] for n in news_list[:3])
    return f"This section highlights the most relevant environmental stories for {col_title}: {lead_titles}."
